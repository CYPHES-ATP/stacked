# CYPHES installer for Windows (PowerShell).
#
# Usage (one-liner):
#   irm https://github.com/CYPHES-ATP/node/releases/latest/download/install.ps1 | iex
#
# Or download and run locally:
#   Invoke-WebRequest https://github.com/CYPHES-ATP/node/releases/latest/download/install.ps1 -OutFile install.ps1
#   .\install.ps1
...
Options:
    ...
    -InstallDir <path>      Override install location (default: %USERPROFILE%\.cyphes\bin)
    -NoModifyPath           Don't add the install dir to user PATH

Examples:
    irm https://github.com/CYPHES-ATP/node/releases/latest/download/install.ps1 | iex
    .\install.ps1 -Version 0.1.0
    .\install.ps1 -Binary C:\path\to\cyphes.exe
"@
}
...
if ([string]::IsNullOrEmpty($InstallDir)) {
    $InstallDir = Join-Path $env:USERPROFILE ".cyphes\bin"
}
...
function Resolve-Version {
    ...
    try {
        $resp = Invoke-RestMethod -UseBasicParsing -Uri "https://api.github.com/repos/$Repo/releases/latest" -Headers @{ 'User-Agent' = 'cyphes-installer' }
        $tag = $resp.tag_name
...
function Check-Existing($desiredVersion) {
    $existing = Get-Command cyphes -ErrorAction SilentlyContinue
    if ($null -eq $existing) { return }
    try {
        $vline = (& cyphes --version) 2>&1 | Select-Object -First 1
    ...
    Write-Muted "Found existing cyphes at $($existing.Source) (v$installed) - upgrading to v$desiredVersion"
}

# ----- Download & extract -----
function Download-And-Install($desiredVersion, $arch) {
    $archive = "cyphes-windows-$arch.zip"
    $url = "https://github.com/$Repo/releases/download/v$desiredVersion/$archive"
    $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("cyphes-install-" + [System.Guid]::NewGuid().ToString('N'))
    ...
    Write-Info "Installing cyphes v$desiredVersion (windows-$arch)"
    ...
    $extractedExe = Join-Path $extractDir 'cyphes.exe'
    if (-not (Test-Path $extractedExe)) {
        Write-Err "Archive did not contain expected binary 'cyphes.exe'"
...
function Install-FromBinary {
    ...
    Write-Info "Installing cyphes from $script:Binary"
    Install-Binary $script:Binary
}

function Install-Binary($source) {
    $target = Join-Path $InstallDir 'cyphes.exe'

    # The currently running cyphes.exe (if any) holds an exclusive file lock on
...
function Add-ToUserPath {
    ...
    # Make it visible in this session too so cyphes --version works immediately.
...
Write-Host ""
Write-Success "cyphes is installed!"
Write-Host ""
...
Write-Success "  cyphes             "
Write-Muted  "  # or"
Write-Success "  cyphes -p `"...`"      "
Write-Host   ""
