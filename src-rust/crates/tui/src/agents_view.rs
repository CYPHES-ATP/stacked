...
use crate::overlays::{
    begin_modal_buf, modal_header_line_area, render_modal_title_buf, CYPHES_ACCENT,
...
/// A defined agent (from .cyphes/agents/*.md or plugin).
#[derive(Debug, Clone)]
...
/// Load agent definitions from `.cyphes/agents/` in project root and home dir.
pub fn load_agent_definitions(project_root: &std::path::Path) -> Vec<AgentDefinition> {
    let mut defs = Vec::new();
    let dirs = [
        dirs::home_dir().map(|h| h.join(".cyphes").join("agents")),
        Some(project_root.join(".cyphes").join("agents")),
    ];
    ...
    if !state.active_agents.is_empty() {
        ...
        for agent in state.active_agents.iter().take(3) {
            lines.push(Line::from(vec![
                Span::styled(" ", Style::default().fg(CYPHES_MUTED)),
                Span::styled(agent.name.clone(), Style::default().fg(CYPHES_TEXT)),
                Span::styled(
...
fn render_agent_detail(def: &AgentDefinition, area: Rect, buf: &mut Buffer) {
    let mut lines = Vec::new();
    lines.push(Line::from(vec![
        Span::styled(" Name       ", Style::default().fg(CYPHES_MUTED)),
        Span::styled(
            def.name.clone(),
            Style::default()
                .fg(CYPHES_TEXT)
    ...
    lines.push(Line::from(vec![
        Span::styled(" Model      ", Style::default().fg(CYPHES_MUTED)),
        Span::raw(def.model.as_deref().unwrap_or("default").to_string()),
    ]));
    if let Some(mem) = &def.memory_scope {
        lines.push(Line::from(vec![
            Span::styled(" Memory     ", Style::default().fg(CYPHES_MUTED)),
            Span::raw(mem.clone()),
        ]));
    }
    if !def.tools.is_empty() {
        lines.push(Line::from(vec![
            Span::styled(" Tools      ", Style::default().fg(CYPHES_MUTED)),
            Span::raw(def.tools.join(", ")),
        ]));
    } else {
        lines.push(Line::from(vec![
            Span::styled(" Tools      ", Style::default().fg(CYPHES_MUTED)),
            Span::styled("All tools", Style::default().fg(CYPHES_MUTED)),
        ]));
    }
...
fn render_agent_editor(state: &AgentsMenuState, area: Rect, buf: &mut Buffer) {
    ...
    let selected_style = Style::default()
        ...
        .bg(CYPHES_ACCENT)
        .add_modifier(Modifier::BOLD);
    let normal_style = Style::default().fg(CYPHES_TEXT);
...
fn agent_list_row(title: String, meta: String, selected: bool, width: u16) -> Line<'static> {
    ...
    let title_style = if selected {
        ...
        Style::default().fg(CYPHES_TEXT).bg(bg)
    };
    let meta_style = if selected {
        Style::default().fg(Color::Rgb(248, 220, 236)).bg(bg)
    } else {
        Style::default().fg(CYPHES_MUTED).bg(bg)
    };
    let mut spans = vec![
