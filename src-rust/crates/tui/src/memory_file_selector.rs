...
impl MemoryFileSelectorState {
    ...
    /// - User:    `~/.cyphes/AGENTS.md`
    /// - Project: `{project_root}/AGENTS.md`
    /// - Local:   `{project_root}/.cyphes/AGENTS.md`
    ///
    ...
    pub fn open(&mut self, project_root: &std::path::Path) {
        ...
        // User-level: ~/.cyphes/AGENTS.md
        let user_path = cyphes_core::config::Settings::config_dir().join("AGENTS.md");
        let user_display = {
        ...
        // Local-level: {project_root}/.cyphes/AGENTS.md
        let local_path = project_root.join(".cyphes").join("AGENTS.md");
...
pub fn render_memory_file_selector(
    ...
    lines.push(Line::from(vec![
        Span::styled(" Memory", Style::default().fg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)),
        Span::styled(" — choose a file", Style::default().fg(CYPHES_MUTED)),
        Span::styled(
    ...
    for (i, file) in state.files.iter().enumerate() {
        ...
        let new_tag = if !file.exists {
            Span::styled(" (new)", Style::default().fg(CYPHES_MUTED))
        } else {
            Span::raw("")
        };
