...
pub fn render_diff_dialog(state: &mut DiffViewerState, area: Rect, buf: &mut Buffer) {
    ...
    if state.files.is_empty() {
        ...
        Paragraph::new(vec![
            ...
            Line::from(vec![Span::styled(
                empty,
                Style::default().fg(CYPHES_TEXT).add_modifier(Modifier::ITALIC),
            )]),
            Line::from(""),
            Line::from(vec![Span::styled(
    ...
    let divider: Vec<Line<'static>> = (0..layout.body_area.height)
        .map(|_| Line::from(Span::styled("│", Style::default().fg(CYPHES_MUTED))))
        .collect();
...
fn render_file_list(state: &DiffViewerState, area: Rect, buf: &mut Buffer) {
    ...
    let header = Line::from(vec![
        Span::styled(
            " Files",
            Style::default()
                .fg(if focused { CYPHES_ACCENT } else { CYPHES_TEXT })
    ...
    for (i, file) in state.files[start..end].iter().enumerate() {
        ...
        let (stats, stats_color) = if file.binary {
            ("binary".to_string(), CYPHES_MUTED)
        } else if file.is_new_file {
            (format!("new  +{}", file.added), Color::Yellow)
        } else {
            (format!("+{} -{}", file.added, file.removed), CYPHES_MUTED)
        };
...
fn render_diff_detail(state: &DiffViewerState, area: Rect, buf: &mut Buffer) {
    ...
    if file.binary {
        Paragraph::new("Binary file — no diff available")
            .style(Style::default().fg(CYPHES_MUTED))
            .render(inner, buf);
        return;
    }
    ...
    for change in diff.iter_all_changes() {
        ...
        match change.tag() {
            ChangeTag::Equal => {
                ...
                new_spans.push(Span::styled(s, Style::default().fg(CYPHES_TEXT)));
            }
            ChangeTag::Delete => {
                old_spans.push(Span::styled(
                    s,
