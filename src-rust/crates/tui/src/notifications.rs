...
use crate::overlays::{
    CYPHES_ACCENT, CYPHES_MUTED, CYPHES_PANEL_BORDER, CYPHES_TEXT,
};
...
impl NotificationKind {
    pub fn color(&self) -> Color {
        match self {
            NotificationKind::Info => CYPHES_ACCENT,
...
pub fn render_notification_banner(frame: &mut Frame, queue: &NotificationQueue, area: Rect) {
    ...
    let mut row0_spans = vec![
        ...
        Span::styled(message, Style::default().fg(CYPHES_TEXT)),
    ];
    if notif.dismissible {
        row0_spans.push(Span::styled(esc_hint.to_string(), Style::default().fg(CYPHES_MUTED)));
    }
    ...
    let progress_line = if let Some(exp) = notif.expires_at {
        ...
        Line::from(vec![
            ...
            Span::styled(empty, Style::default().fg(CYPHES_MUTED)),
            Span::raw(" "),
        ])
    } else {
            ...
            for row in 0..toast_height {
                if toast_area.y + row < buf.area().bottom() {
                    if let Some(cell) = buf.cell_mut((right_x, toast_area.y + row)) {
                        cell.set_bg(bg);
                        cell.set_fg(CYPHES_PANEL_BORDER);
                        cell.set_char('▐');
                    }
                }
            }
