...
use crate::overlays::{
    centered_rect, render_dark_overlay_buf, render_dialog_bg_buf, CYPHES_ACCENT, CYPHES_MUTED,
    CYPHES_PANEL_BG, CYPHES_PANEL_BORDER, CYPHES_TEXT,
};
...
pub fn render_mcp_view(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let title = Line::from(vec![
        Span::styled(" MCP", Style::default().fg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)),
        Span::styled(
    ...
    Paragraph::new(title)
        .style(Style::default().bg(CYPHES_PANEL_BG).fg(CYPHES_TEXT))
        .render(layout[0], buf);
    ...
    let footer = Line::from(vec![
        Span::styled(" Tab ", Style::default().fg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("switch pane  "),
        Span::styled(" / ", Style::default().fg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("filter tools  "),
        Span::styled(" e ", Style::default().fg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("error detail  "),
        Span::styled(" a ", Style::default().fg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("auth  "),
        Span::styled(" r ", Style::default().fg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("reconnect"),
    ]);
    Paragraph::new(footer)
        .style(Style::default().bg(CYPHES_PANEL_BG).fg(CYPHES_MUTED))
        .render(layout[2], buf);
}
...
fn render_server_list(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let border_style = if focused {
        ...
        Style::default().fg(CYPHES_PANEL_BORDER)
    };
    Block::default()
        .title(Span::styled(
            " Servers ",
            Style::default()
                .fg(if focused { CYPHES_ACCENT } else { CYPHES_MUTED })
        ...
        *row += 1;
        for (idx, server) in group {
            ...
            let line = Line::from(vec![Span::styled(
                row_text,
                if sel {
                    Style::default().fg(Color::Black).bg(CYPHES_ACCENT).add_modifier(Modifier::BOLD)
                } else {
...
fn render_tool_list(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let search_line = Line::from(vec![
        Span::styled("/ ", Style::default().fg(CYPHES_ACCENT)),
        Span::styled(
            if state.tool_search.is_empty() { "filter tools".to_string() } else { state.tool_search.clone() },
            Style::default().fg(if state.tool_search.is_empty() { CYPHES_MUTED } else { CYPHES_TEXT }),
        ),
    ]);
