// upgrade.rs — `cyphes upgrade` subcommand.
//
...
// so that an `upgrade` from inside CYPHES feels identical to a fresh install.
//
...
const REPO: &str = "CYPHES-ATP/node";
const APP: &str = "cyphes";
...
pub async fn run_upgrade(args: &[String]) -> Result<()> {
    ...
    println!("Run `cyphes --version` in a new shell to verify.");
    Ok(())
}

fn print_help() {
    println!(
        "Usage: cyphes upgrade [options]\n\n\
         Options:\n\
           -v, --version <v>   Install a specific version (default: latest)\n\
           -f, --force         Reinstall even if already up to date\n\
           -h, --help          Show this help\n\n\
         Downloads the latest cyphes release from GitHub and replaces this\n\
         binary in place. Settings in ~/.cyphes are preserved."
    );
}
...
async fn fetch_latest_version() -> Result<String> {
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent(format!("cyphes-upgrade/{}", env!("CARGO_PKG_VERSION")))
        .build()?;
...
fn swap_binary(current: &Path, new: &Path) -> Result<()> {
    #[cfg(target_os = "windows")]
    {
        ...
        if let Err(e) = std::fs::copy(new, current) {
            // Try to roll back the rename so the user isn't left without cyphes.
...
fn tempdir_for_upgrade() -> Result<PathBuf> {
    ...
    let now = std::time::SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    let dir = base.join(format!("cyphes-upgrade-{}-{}", pid, now));
