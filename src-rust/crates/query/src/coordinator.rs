...
pub const COORDINATOR_ENV_VAR: &str = "CYPHES_COORDINATOR_MODE";
...
/// Tools that workers are allowed to use in simple mode (CYPHES_SIMPLE=1).
...
pub fn filter_tools_for_mode<'a>(
    tools: &'a [Box<dyn cyphes_tools::Tool>],
    mode: AgentMode,
) -> Vec<&'a Box<dyn cyphes_tools::Tool>> {
    match mode {
