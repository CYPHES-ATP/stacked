...
/// Scan `project_root` for skill definitions in `.cyphes/skills/` and the bundled
...
pub async fn prefetch_skills(project_root: &Path, index: SharedSkillIndex) {
    ...
    // 1. User-defined skills: ~/.cyphes/skills/*.md + {project_root}/.cyphes/skills/*.md
    let search_dirs: Vec<std::path::PathBuf> = {
        let mut dirs = Vec::new();
        if let Some(home) = dirs::home_dir() {
            dirs.push(home.join(".cyphes").join("skills"));
        }
        dirs.push(project_root.join(".cyphes").join("skills"));
        dirs
    };
