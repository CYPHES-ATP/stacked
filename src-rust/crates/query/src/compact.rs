...
use cyphes_api::{AnthropicStreamEvent, ApiMessage, CreateMessageRequest, StreamAccumulator, StreamHandler, SystemPrompt};
use cyphes_core::error::ClaudeError;
use cyphes_core::types::{ContentBlock, Message, MessageContent, Role};
...
fn estimate_block_chars(block: &ContentBlock) -> usize {
    match block {
        ...
        ContentBlock::ToolResult { content, .. } => match content {
            cyphes_core::types::ToolResultContent::Text(t) => t.len(),
            cyphes_core::types::ToolResultContent::Blocks(blocks) => {
                ...
                match block {
                    ...
                    ContentBlock::ToolResult { tool_use_id, content, is_error } => {
                        let result_text = match content {
                            cyphes_core::types::ToolResultContent::Text(t) => t.as_str().to_string(),
                            cyphes_core::types::ToolResultContent::Blocks(_) => "[complex content]".to_string(),
                        };
...
pub struct CompactResult {
    ...
    pub messages: Vec<cyphes_core::types::Message>,
...
pub fn snip_compact(messages: Vec<cyphes_core::types::Message>, keep_n_newest: usize) -> (Vec<cyphes_core::types::Message>, u64) {
    let total = messages.len();
...
pub fn calculate_messages_to_keep_index(messages: &[cyphes_core::types::Message], token_budget: u64) -> usize {
...
/// `reactiveCompact.ts`.
fn strip_images(messages: Vec<cyphes_core::types::Message>) -> Vec<cyphes_core::types::Message> {
...
/// `cyphes_core::feature_gates::is_feature_enabled("reactive_compact")` is true.
///
/// The `cancel` token is checked before the API call so the user can abort
/// a long-running compact.
pub async fn reactive_compact(
    messages: Vec<cyphes_core::types::Message>,
    client: &cyphes_api::AnthropicClient,
    config: &crate::QueryConfig,
    cancel: tokio_util::sync::CancellationToken,
    recently_modified: &[std::path::PathBuf],
) -> Result<CompactResult, cyphes_core::error::ClaudeError> {
    if cancel.is_cancelled() {
        return Err(cyphes_core::error::ClaudeError::Cancelled);
    }
    ...
    for path in recently_modified.iter().take(MAX_FILES * 3) {
        ...
        new_messages.push(cyphes_core::types::Message::user(text));
        injected += 1;
    }
...
pub async fn context_collapse(
    ...
    for msg in &messages {
        let role = match msg.role {
            cyphes_core::types::Role::User => "Human",
    ...
    if summary_text.is_empty() {
        return Err(cyphes_core::error::ClaudeError::Other(
            "Context-collapse summary was empty".to_string(),
        ));
    }

    // Keep only: the synthetic summary + the most recent user turn.
    let collapse_notice = cyphes_core::types::Message::user(format!(
    ...
    let last_user = messages
        .iter()
        .rev()
        .find(|m| m.role == cyphes_core::types::Role::User)
        .cloned();
...
pub fn collapse_read_tool_results(messages: Vec<cyphes_core::types::Message>) -> Vec<cyphes_core::types::Message> {
    use cyphes_core::types::{ContentBlock, MessageContent, ToolResultContent};
...
pub fn collapse_search_results(messages: Vec<cyphes_core::types::Message>) -> Vec<cyphes_core::types::Message> {
    use cyphes_core::types::{ContentBlock, MessageContent, ToolResultContent};
    ...
    let mut result: Vec<cyphes_core::types::Message> = messages
        .into_iter()
        .rev()
        .map(|mut msg| {
            if let MessageContent::Blocks(ref mut blocks) = msg.content {
...
mod tests {
    use super::*;
    use cyphes_core::types::{Message, Role};
