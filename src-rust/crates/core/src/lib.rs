...
pub mod error {
    ...
    /// The unified error type for CYPHES.
    #[derive(Error, Debug)]
...
pub mod config {
    ...
    impl Config {
        ...
        /// user-defined styles loaded from `~/.cyphes/output-styles/`.
        ...
        /// Async variant: also checks `~/.cyphes/oauth_tokens.json`.
    ...
    impl Settings {
        /// The per-user configuration directory (`~/.cyphes`).
        ...
        /// Walk up from `cwd` looking for `.cyphes/settings.json` or
        /// `.cyphes/settings.jsonc`.
        async fn find_project_settings(cwd: &std::path::Path) -> Option<Self> {
            ...
            loop {
                ...
                for name in &["settings.json", "settings.jsonc"] {
                    let candidate = dir.join(".cyphes").join(name);
...
pub mod constants {
    ...
    pub const CONFIG_DIR_NAME: &str = ".cyphes";
...
pub mod context {
    ...
    impl ContextBuilder {
        ...
        async fn find_and_read_claude_md(&self) -> Option<String> {
            ...
            // Global ~/.cyphes/AGENTS.md
            if let Some(home) = dirs::home_dir() {
                let global_claude_md =
                    home.join(".cyphes").join(crate::constants::CLAUDE_MD_FILENAME);
...
pub mod history {
    ...
    /// Save a session to `~/.cyphes/sessions/<id>.json`.
...
pub mod oauth {
    ...
    // `cyphes_core::oauth_config` for the matching request-time headers and
    // system-prompt prefix wired into `cyphes_api::AnthropicClient`).
    ...
    /// Persisted OAuth tokens (saved to `~/.cyphes/oauth_tokens.json`).
    #[derive(Debug, Clone, Serialize, Deserialize, Default)]
    ...
    impl OAuthTokens {
        ...
        /// `~/.cyphes/accounts/anthropic/<profile_id>/oauth_tokens.json`.
        ...
        /// legacy `~/.cyphes/oauth_tokens.json` (auto-migrating it into a
