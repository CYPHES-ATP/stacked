...
// Anthropic's auth server accepts Claude Pro/Max tokens through CYPHES.
...
// `cyphes_api::client::AnthropicClient` and is required for these tokens to
...
/// OpenAI Codex OAuth tokens, persisted to ~/.cyphes/codex_tokens.json
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Legacy single-file path: `~/.cyphes/codex_tokens.json`. Kept for
/// backward-compat reads when no account registry exists.
fn codex_tokens_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|h| h.join(".cyphes").join("codex_tokens.json"))
}

/// Save Codex OAuth tokens for a named profile under
/// `~/.cyphes/accounts/codex/<profile_id>/codex_tokens.json`.
...
/// or by setting `CYPHES_USE_OPENAI=1` with a manually stored token.
