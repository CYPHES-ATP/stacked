...
//   - Entries are written to `~/.cyphes/history.jsonl` via O_APPEND.
...
//     `~/.cyphes/pastes/<sha256-hex>` and referenced by hash.
...
/// Persist `text` to `~/.cyphes/pastes/<hash>`.  Fire-and-forget.
...
/// Read text from `~/.cyphes/pastes/<hash>`, returning `None` if missing.
...
/// Append `entry` to `~/.cyphes/history.jsonl`.
///
/// The call is fire-and-forget: it spawns a background Tokio task and returns
/// immediately.  If `CYPHES_SKIP_PROMPT_HISTORY` is truthy the call is
/// a no-op.
pub fn add_to_history(entry: HistoryEntry) {
    if std::env::var("CYPHES_SKIP_PROMPT_HISTORY")
...
/// Read `~/.cyphes/history.jsonl`, filter by `project`, and return up to
