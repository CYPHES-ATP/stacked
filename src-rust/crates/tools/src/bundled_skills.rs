...
- User:    ~/.cyphes/settings.json
- Project: .cyphes/settings.json
- Local:   .cyphes/settings.local.json
...
6. Report: what was verified, what passed, what failed or is uncertain."#,
    ...
    BundledSkill {
        name: "update-config",
        description: "Configure CYPHES settings (hooks, permissions, env vars, behaviours) via settings.json.",
...
Modify CYPHES configuration by updating settings.json files.

## Settings File Locations

| File | Scope | Use For |
|------|-------|---------|
| `~/.cyphes/settings.json` | Global | Personal preferences for all projects |
| `.cyphes/settings.json` | Project | Team-wide hooks, permissions, plugins |
| `.cyphes/settings.local.json` | Project (local) | Personal overrides for this project |
