// ConfigTool: get or set CYPHES configuration settings at runtime.
//
// Reads from and persists to ~/.cyphes/settings.json.
...
impl Tool for ConfigTool {
    ...
    fn description(&self) -> &str {
        "Get or set CYPHES configuration settings. Omit 'value' to read the current value. \
         Supported settings: model, max_tokens, verbose, permission_mode, auto_compact. \
         Changes persist to ~/.cyphes/settings.json."
    }
    ...
    async fn execute(&self, input: Value, _ctx: &ToolContext) -> ToolResult {
        ...
        let mut settings = match cyphes_core::config::Settings::load().await {
            Ok(s) => s,
...
fn permission_mode_str(mode: &cyphes_core::config::PermissionMode) -> &'static str {
    use cyphes_core::config::PermissionMode;
    match mode {
