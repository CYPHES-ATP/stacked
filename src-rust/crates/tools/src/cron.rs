...
// Durable tasks are persisted to `~/.cyphes/scheduled_tasks.json`.
//
...
/// Path to `~/.cyphes/scheduled_tasks.json`.
fn scheduled_tasks_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".cyphes").join("scheduled_tasks.json"))
}
...
async fn ensure_store_loaded() {
    ...
    *init = true;
    ...
    // Load from ~/.cyphes/scheduled_tasks.json if it exists.
...
impl Tool for CronCreateTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "durable": {
                    "type": "boolean",
                    "description": "true = persist to .cyphes/scheduled_tasks.json; false (default) = session only"
                }
            },
    ...
    async fn execute(&self, input: Value, _ctx: &ToolContext) -> ToolResult {
        ...
        // Persist to ~/.cyphes/scheduled_tasks.json for durable tasks.
        if params.durable {
        ...
        let where_note = if params.durable {
            "Persisted to ~/.cyphes/scheduled_tasks.json"
        } else {
...
/// Persist all durable tasks to `~/.cyphes/scheduled_tasks.json`.
