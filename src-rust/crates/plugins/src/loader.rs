...
/// 1. `~/.cyphes/plugins/<name>/`  — user-global plugins
/// 2. `<project>/.cyphes/plugins/<name>/`  — project-local plugins
...
/// Return the default user-level plugins directory: `~/.cyphes/plugins`.
pub fn default_user_plugins_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".cyphes").join("plugins"))
}

/// Return the project-level plugins directory: `<project>/.cyphes/plugins`.
pub fn project_plugins_dir(project_dir: &Path) -> PathBuf {
    project_dir.join(".cyphes").join("plugins")
}
