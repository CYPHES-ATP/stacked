// cyphes-plugins: Plugin runtime for the CYPHES CLI.
//
...
///
/// Search order:
/// 1. `~/.cyphes/plugins/`  (user-global)
/// 2. `<project_dir>/.cyphes/plugins/`  (project-local)
...
///
/// Copies the plugin directory into `~/.cyphes/plugins/` and returns the
...
mod tests {
    ...
    async fn load_plugins_finds_project_plugin() {
        let tmp = TempDir::new().unwrap();
        let plugin_dir = tmp.path().join(".cyphes").join("plugins").join("test-plugin");
