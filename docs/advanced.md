...
This document covers CYPHES's advanced capabilities beyond basic coding assistance.
...
```
cyphes --thinking <tokens>    Set a specific token budget for thinking
cyphes --effort <level>       Set the effort level (low/medium/high/max)
```
...
**Environment variable override:** `CLAUDE_CODE_EFFORT_LEVEL` overrides the persisted setting for the current process. If this variable is set and conflicts with a `/effort` command, CYPHES displays a warning.
...
CYPHES tracks token usage after every model turn. When usage crosses the auto-compact threshold — which is the effective context window size minus a 13,000-token buffer — it runs `compactConversation` to summarise the history and replaces the messages with a compact summary plus any trailing context.
...
```bash
DISABLE_COMPACT=1 cyphes
```

**Override the threshold window (for testing):**

```bash
CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=80 cyphes
```
...
Sessions are stored as JSONL files under `~/.cyphes/projects/<base64url(project-root)>/<session-id>.jsonl`. Each line in the file is a JSON object representing a message or event in the conversation. The per-session metadata index lives at `~/.cyphes/sessions/<id>.json`.
...
The `parentUuid` field forms a linked chain that allows CYPHES to reconstruct the conversation tree. `/fork` rewrites all UUIDs while preserving the chain structure.
...
The goal system lets CYPHES work autonomously across multiple turns toward a single, verifiable objective. Instead of prompting repeatedly, you set the goal once and CYPHES iterates until the goal is complete, paused, or the built-in runaway guard fires.
...
Once a goal is set, CYPHES begins working immediately. It continues across turns without waiting for user input until one of these conditions is met:
...
CYPHES displays both to the user before marking the goal complete. The model is expected to genuinely audit the outcome before calling; calling without real evidence is rejected.
...
```bash
CYPHES_GOALS=0 cyphes
```

Set `CYPHES_GOALS=0` in the environment to completely disable goal-related commands and the `GoalCompleteTool`. Useful in environments where autonomous multi-turn execution is undesirable.
...
Configuration persists to `~/.cyphes/settings.json` under `managed_agents`.
...
Headless mode runs CYPHES non-interactively, suitable for scripts, CI pipelines, and programmatic orchestration.

### --print flag

```bash
cyphes --print "refactor this function to use async/await"
cyphes -p "summarise the changes in this PR"
```
...
```bash
cat my_prompt.txt | cyphes --print
echo "explain this code" | cyphes -p
```

### --output-format

```bash
cyphes --print --output-format json "..."
cyphes --print --output-format stream-json --verbose "..."
```
...
```bash
cyphes --max-budget-usd 2.00 "..."   # Stop after spending $2.00
cyphes --max-turns 10 "..."          # Stop after 10 model turns
cyphes --max-tokens 50000 "..."      # Stop after 50,000 output tokens
```

When a limit is reached, CYPHES exits with a corresponding error message:
...
Every CYPHES user gets a persistent companion derived deterministically from their user ID. The companion appears as a small sprite in the terminal UI and occasionally comments on activity.
...
CYPHES includes a legacy-compatible remote bridge mode. This keeps a WebSocket connection open so a remote UI can send prompts and receive streaming responses.
...
- **In-process bridge** — the WebSocket lives inside the CYPHES process. If the process dies, the connection is lost.
- **Daemon bridge** — the WebSocket lives in a parent daemon process. The agent can be respawned while the remote session stays connected. This is the `connectRemoteControl` SDK primitive.

SSH sessions work similarly: `cyphes --ssh` enables a remote-accessible session that can be connected to from another machine.

---

## AGENTS.md hierarchical memory

CYPHES reads instruction files from the filesystem before every session and whenever a relevant file changes. The lookup order is:

1. **Managed** — `/etc/cyphes/AGENTS.md` and `/etc/cyphes/CLAUDE.md` (administrator-controlled, always loaded).
2. **User** — `~/.cyphes/AGENTS.md` then `~/.cyphes/CLAUDE.md` (personal global instructions).
3. **Project** — `AGENTS.md`, `CLAUDE.md`, `.cyphes/AGENTS.md`, and `.cyphes/CLAUDE.md` in each directory from the filesystem root down to the current working directory. Files are loaded from the root toward the CWD so that parent-directory rules are visible when processing child-directory rules.
...
Controls how CYPHES formats its responses. Available styles vary by configuration; the command opens an interactive picker when called without arguments.
...
```bash
cyphes --add-dir /path/to/additional/project "..."
```

Grants CYPHES read access to an additional directory outside the working directory. Useful when a task spans multiple repositories or when config files live outside the project root.
