# CYPHES Slash Commands Reference

This document is the complete reference for every slash command available in CYPHES Node. Commands are invoked by typing `/command-name` at the REPL prompt.
...
Commands are registered in a priority-ordered registry. When you type a command name, CYPHES resolves it through this chain:
...
Exit the CYPHES REPL. Equivalent to pressing `Ctrl+D`. Unsaved session state is flushed before exit.
...
The advisor model persists to `~/.cyphes/settings.json` under `advisorModel`. Model IDs must start with `claude-` or contain a `/` (provider/model format).
...
Toggle fast mode. In fast mode, CYPHES switches to the active provider's smaller, faster model for quick responses. Useful when you want rapid answers and deep reasoning is not required.
...
Setting persists to `~/.cyphes/ui-settings.json`.
...
View or modify CYPHES configuration values. Without arguments, renders an interactive settings panel. With arguments, acts as a key-value accessor.
...
Open the interactive keybinding configurator. Displays all bound actions with their current shortcuts. Select an action to rebind it. Changes are written to `~/.cyphes/keybindings.json`.
...
Open CYPHES privacy settings. Launches a browser to the Anthropic privacy portal where you can review data usage preferences, conversation retention, and account privacy options.
...
Open the interactive theme picker. Preview and select a color theme for the CYPHES TUI.
...
Run the terminal capability detection and setup wizard. Checks for true-color support, font ligatures, Unicode rendering, and configures CYPHES accordingly.
...
Show file diffs for changes made during the current session. Displays a unified diff of all files CYPHES has written or edited since the session started.
...
Undo file changes made during the current session. Restores files to their state before CYPHES's last write operation. Can be called multiple times to step further back.
...
Initialize CYPHES project configuration in the current directory. Creates a `CLAUDE.md` file that acts as persistent project-level context injected at the start of every session.
...
Set a durable multi-turn autonomous goal. When a goal is active, CYPHES continues working across turns until the goal is marked complete, paused, or a 200-turn runaway guard fires. Designed for complex, sustained tasks that would otherwise require repeated manual re-prompting.
...
When the model believes the goal has been achieved, it calls the `GoalComplete` tool with an audit summary and evidence. Goals can be disabled globally by setting `CYPHES_GOALS=0` in your environment.
...
Model format: `provider/model` (e.g., `anthropic/claude-opus-4-6`, `openai/gpt-4o`). Configuration persists to `~/.cyphes/settings.json` under `managed_agents`.
...
To activate an agent, start CYPHES with `--agent <name>`. See [agents.md](./agents.md) for defining custom agents.
...
List and manage skills. Skills are bundled prompt-commands that extend CYPHES's capabilities without writing code. They appear alongside built-in commands in the registry.
...
Browser automation via Chrome DevTools Protocol (CDP). Connects to a running Chrome or Chromium instance and lets CYPHES control it — navigate pages, click elements, fill forms, evaluate JavaScript, and take screenshots.
...
CYPHES supports **multiple named accounts per provider** — Anthropic (Claude.ai or Console) and Codex (OpenAI ChatGPT subscription). Each login creates a profile under `~/.cyphes/accounts/<provider>/<id>/` and the registry at `~/.cyphes/accounts.json` tracks which one is active.
...
Display a QR code for supported mobile handoff flows. The `session` subcommand generates a QR code linking directly to an active remote CYPHES session.

```
/mobile             — show QR code for the compatible mobile endpoint
...
Opens the CYPHES sticker page in your default browser. Falls back to printing the URL if no browser can be launched.
...
Run the CYPHES diagnostics suite. Checks configuration integrity, provider connectivity, tool availability, MCP server health, and reports any issues.
...
Display the current CYPHES version string and build metadata.
...
Thinking traces appear when the model uses extended thinking mode (see `/thinking`). If no traces are found, CYPHES suggests enabling extended thinking.
