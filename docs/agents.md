...
CYPHES has a named-agent system that lets you select a pre-configured persona with its own tool permissions, model, system prompt, and turn budget. For larger tasks it also supports a coordinator mode where a top-level agent orchestrates a pool of parallel worker agents.

---

## Built-in Named Agents

Three agents ship by default. Their definitions can be overridden per-user in `~/.cyphes/settings.json`.
...
```
cyphes --agent build "implement the OAuth2 login flow"
cyphes --agent plan "analyze the database schema and suggest improvements"
cyphes --agent explore "find all usages of the deprecated config API"
```

The `--agent` flag can be combined with `--provider` and `--model`:

```
cyphes --agent plan --provider openai --model o3 "review this architecture"
```
...
Define custom agents in `~/.cyphes/settings.json` under the `agents` key. Custom definitions override built-in agents of the same name.
...
```
cyphes --agent review "check the authentication module for security issues"
cyphes --agent test-writer "write tests for the payment processor"
```
...
Set the `CYPHES_COORDINATOR_MODE` environment variable to `1` before launching:

```bash
CYPHES_COORDINATOR_MODE=1 cyphes "refactor the entire authentication subsystem"
```

Or within a shell session:

```bash
export CYPHES_COORDINATOR_MODE=1
cyphes
```
...
When coordinator mode is active, CYPHES injects a coordinator system prompt that instructs the model to orchestrate rather than act directly. The recommended workflow is:
...
In simple mode (`CYPHES_SIMPLE=1`), workers are further restricted to `["Bash", "Read", "Edit"]`.
...
```bash
CYPHES_COORDINATOR_MODE=1 cyphes \
...
When resuming a saved session, CYPHES detects whether the original session used coordinator mode and automatically sets `CYPHES_COORDINATOR_MODE` to match. A warning is printed when the environment is changed to prevent mode confusion in long-running workflows.
...
|---|---|---|
| **Setup** | `CYPHES_COORDINATOR_MODE=1` | `/managed-agents enable` |
