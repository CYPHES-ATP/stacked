# CYPHES Configuration Reference

CYPHES is configured through a layered system of JSON files, environment
...
The directory `~/.cyphes/` is created automatically on first run if it does
not exist. The file is standard JSON (or JSONC — comments are stripped before
parsing).

### Per-project settings

CYPHES walks up from the current working directory looking for a project-level
settings file. The first file found wins (project settings take precedence over
global settings):

```
<project-root>/.cyphes/settings.json
<project-root>/.cyphes/settings.jsonc
```
...
| `output_style` | string \| null | null | Named output style. Built-in values: `"default"`, `"concise"`, `"verbose"`. Custom styles can be added as Markdown files under `~/.cyphes/output-styles/`. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `custom_system_prompt` | string \| null | null | Replace the default CYPHES system prompt entirely with this text. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `additional_dirs` | array of strings | [] | Additional filesystem paths CYPHES is allowed to read and write. Equivalent to passing `--add-dir` on the command line. |
...
"hooks": {
  ...
  "Stop": [
    { "command": "notify-send 'CYPHES done'", "blocking": false }
  ]
}
```
...
on Unix systems (CYPHES blocks it).
...
```bash
cyphes --permission-mode acceptEdits "refactor the auth module"
cyphes --dangerously-skip-permissions "..."  # equivalent to bypassPermissions
```
...
AGENTS.md files are plain Markdown documents that CYPHES injects into the
...
CYPHES loads AGENTS.md files from four locations. They are processed in the
following order (earlier = higher priority, later content is appended below):

| Scope | Path | Description |
|-------|------|-------------|
| Managed | `~/.cyphes/rules/*.md` | Global policy files. All `.md` files in this directory are loaded in alphabetical order. |
| User | `~/.cyphes/AGENTS.md` | Your personal preferences and instructions, applied to all projects. |
...
| Local | `<project-root>/.cyphes/AGENTS.md` | Local overrides not committed to version control (add `.cyphes/` to `.gitignore`). |
...
`AGENTS.md`. Both names are supported so existing agent-instruction files keep
working during migration to CYPHES.
...
```bash
cyphes --no-claude-md "your prompt"
```
...
CYPHES can send requests to multiple LLM providers. Set the active provider
...
| `CYPHES_PROVIDER` | Active provider. Equivalent to `--provider`. |
| `CYPHES_API_BASE` | Override the API base URL for the active provider. Equivalent to `--api_base`. |
| `CYPHES_GOALS` | Set to `0` to disable the goal system (`/goal` command and `GoalCompleteTool`). |
...
| `CYPHES_BRIDGE_URL` | Enable the remote-control bridge by setting the server URL. |
| `CYPHES_BRIDGE_TOKEN` | Bearer token for the remote-control bridge. |
| `RUST_LOG` | Tracing filter (e.g. `debug`, `cyphes_core=trace`). |
...
Formatters run automatically after CYPHES writes a file whose extension
