# CYPHES Tools Reference

This document is the complete reference for every tool available to the CYPHES agent. Tools are the mechanism by which the model interacts with the outside world — reading files, running commands, searching the web, and coordinating sub-agents.
...
Every tool in CYPHES implements a common `Tool` interface. This interface defines:
...
**Interactive mode** (default REPL): CYPHES presents a confirmation prompt for any tool that lacks a pre-existing approval rule. The user can approve once, approve always (adding a permanent rule), or deny.
...
Model Context Protocol (MCP) tools bridge CYPHES to external MCP servers.
...
Read or write CYPHES configuration values programmatically. Used by commands and tools that need to persist settings.
...
This tool has the highest blast radius of any tool in CYPHES. It requires explicit permission and should only be enabled in controlled environments. All actions are logged in detail.
